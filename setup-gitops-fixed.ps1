...
if (-not $SkipArgoCD) {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] DevOpsify project created" -ForegroundColor Green
    }
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] Applications created (dev, staging, prod)" -ForegroundColor Green
    }
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] ArgoCD UI exposed" -ForegroundColor Green
    }
    ...
    Write-Host "`nGetting ArgoCD UI URL..." -ForegroundColor Yellow
    ...
    Write-Host "`nSkipping ArgoCD installation" -ForegroundColor Gray
}
...
if (-not $SkipMonitoring) {
    ...
    Write-Host "`nGetting Grafana URL..." -ForegroundColor Yellow
    $grafanaIP = ""
    for ($i = 0; $i -lt 30; $i++) {
    ...
    Write-Host "`nSkipping monitoring installation" -ForegroundColor Gray
}
...
if (-not $SkipArgoCD) {
    Write-Host "   [OK] ArgoCD GitOps platform" -ForegroundColor Green
    Write-Host "   [OK] DevOpsify project and applications (dev, staging, prod)" -ForegroundColor Green
}
if (-not $SkipMonitoring) {
    Write-Host "   [OK] Prometheus monitoring" -ForegroundColor Green
